...
def first_page(request):
    ...
    if( request.method == 'POST'):
      ...
      company_df = pd.read_excel('P:/Risk estimator/risk_estimator/risk_estimator/static/company_ranks.xlsx')
      company_df2 = company_df[['company', 'flag', 'Overall_linkedIn_sentiment', 'news_issues', 'News_sentiment', 'date_time']].copy()
      company_df3 = company_df[['company', 'flag', 'Overall_linkedIn_sentiment', 'net_income_actual', 'net_profit_actual', 'quarterly_revenue_actual', 'operating_cash_flow_actual','news_issues', 'News_sentiment', 'date_time']].copy()
        
     
      json_records = company_df3.reset_index().to_json(orient = 'records')
      company_history = []
      company_history = json.loads(json_records)
                
   
               


      if(request.POST.get('scrape') and request.POST.get('company') and request.POST.get('city') ):
...
# Google Scraping to check if google finance data exists for the Company, if yes, grab the link
        ...
        df_excel = pd.read_excel('P:/Risk estimator/risk_estimator/risk_estimator/static/company_ranks.xlsx')
        ...
        dictionary = pd.read_excel("P:/Risk estimator/risk_estimator/risk_estimator/static/dictionary.xlsx")
        ...
        if (len(issues) != 0):
                ...
                if(len(news_issues) != 0):
                        ...
                        if ((Neutral_news > Negative_news) and (Neutral_news > Positive_news)):
                                df.at[0, 'News_sentiment'] = 'Neutral'
        ...
        df_excel.drop(0)
        df = df[0:0]
        df_excel.to_excel('P:/Risk estimator/risk_estimator/risk_estimator/static/company_ranks.xlsx')
        ...
        negatives = []
        selected_company_df = company_df.loc[company_df['company'] == company_name]

        selected_company = selected_company_df.iloc[-1,:]
        
        selected_company.fillna('None', inplace= True)
        print(selected_company)
        if selected_company['flag'] == 'red' :
                if selected_company['net_income'] < 0:
                        negatives.append('Net Income')
                if selected_company['net_profit'] < 0:
                        negatives.append('Net Profit')
                if selected_company['quarterly_revenue'] < 0:
                        negatives.append('Quarterly Revenue')
                if selected_company['operating_cash_flow'] < 0:
                        negatives.append('Operating Cash Flow')
                
                finance_status = 'The following financial metrics are currently in negative for the company: '+ ','.join(negatives)
        if selected_company['flag'] == 'green' :
                
                finance_status = 'All financial metrics are currently positive  for the company'
        
        if selected_company['flag'] == 'Z' :
                finance_status = 'The company was not found on Google Finance'
        
        if selected_company['Overall_linkedIn_sentiment'] == 'None' :
                linked_in = 'The Company was not found on linkedIn'
        if selected_company['Overall_linkedIn_sentiment'] != 'None' :
                linked_in = 'The LinkedIn Sentiment is: '+selected_company['Overall_linkedIn_sentiment']
        if selected_company['news_issues'] == 'None' :
                news_issues = 'No critical issues relevant to the company or city could be retrievd'
                news_sentiment = 'None'
        if selected_company['news_issues'] != 'None' :
                news_issues = selected_company['news_issues']
                news_sentiment = selected_company['News_sentiment']
        ...
        company_history = []
        company_history = json.loads(json_records)
        return render(request,'first_page.html' ,{'content2': company_history, 'finance_status': finance_status, 'linked_in': linked_in, 'news_issues': news_issues, 'news_sentiment': news_sentiment, 'net_income': y_y_net_income2, 'net_profit': y_y_net_profit2, 'net_revenue': y_y_revenue2, 'company': request.POST.get('company'), 'city': request.POST.get('city')})
        ...
        #return render(request,'first_page.html' ,{'content': company_history, 'finance_status': finance_status, 'linked_in': linked_in, 'news_issues': news_issues, 'news_sentiment': news_sentiment})

      else:
        return render(request,'first_page.html', {'content': company_history})
        ...
        company_history = []
        company_history = json.loads(json_records)
        return render(request,'first_page.html', {'content2': company_history})
